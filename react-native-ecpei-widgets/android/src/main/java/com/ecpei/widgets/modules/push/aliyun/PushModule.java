...
public class PushModule extends ReactContextBaseJavaModule {
    ...
    public static void sendEvent(String eventName, @Nullable WritableMap params) {
        if (context == null) {
            ...
            try {
                ...
                ex.printStackTrace();
                Log.d("PushModule", "sendEvent: " + ex.getMessage());
            }
        }
    }
    ...
    @ReactMethod
    public void addAlias(String account,final Callback callback){
        PushServiceFactory.getCloudPushService().addAlias(account, new CommonCallback() {
            @Override
            public void onSuccess(String s) {
                callback.invoke("add account success");
            }

            @Override
            public void onFailed(String s, String s1) {
                callback.invoke("add account failed. errorCode:" + s + ", errorMsg:" +s1);
            }
        });
    }

    @ReactMethod
    public void removeAlias(String account,final Callback callback){
        PushServiceFactory.getCloudPushService().removeAlias(account, new CommonCallback() {
            @Override
            public void onSuccess(String s) {
                callback.invoke("remove account success");
            }

            @Override
            public void onFailed(String s, String s1) {
                callback.invoke("remove account failed. errorCode:" + s + ", errorMsg:" +s1);
            }
        });
    }
    ...
    @ReactMethod
    public void clearAllNotifications(String pushID){
